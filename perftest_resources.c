...
struct perftest_parameters* duration_param2;
 ...
 *
 ******************************************************************************/
int *run_iter_bw_mt(void *context) {
//int *bufferRead (void *context) {
    struct bw_args *b = context;
	run_iter_bw(b->ctx,b->user_param);
	printf("multithread\n\n");
 ...
 *
 ******************************************************************************/
int run_iter_bw_infinitely_dual(struct pingpong_context *ctx1,struct perftest_parameters *user_param1, struct pingpong_context *ctx2,struct perftest_parameters *user_param2)
{
	uint64_t		totscnt = 0;
	uint64_t		totccnt = 0;
	int 			i = 0;
	...
	uint64_t		*scnt_for_qp = NULL;
	struct ibv_wc 		*wc = NULL;
	int 			num_of_qps1 = user_param1->num_of_qps;
	int 			num_of_qps2 = user_param2->num_of_qps;
	int 			return_value = 0;
	int 			single_thread_handler;

  printf("run_iter_bw_infinitely_dual\n\n");
	ALLOCATE(wc ,struct ibv_wc ,CTX_POLL_BATCH);
	ALLOCATE(scnt_for_qp,uint64_t,user_param1->num_of_qps);
	memset(scnt_for_qp,0,sizeof(uint64_t)*user_param1->num_of_qps);

	ALLOCATE(scnt_for_qp,uint64_t,user_param2->num_of_qps);
	memset(scnt_for_qp,0,sizeof(uint64_t)*user_param2->num_of_qps);


	duration_param=user_param1;
	duration_param2=user_param2;
	...
	alarm(user_param1->duration);
	user_param1->iters = 0;
	alarm(user_param2->duration);
	user_param2->iters = 0;

	/* Will be 0, in case of Duration (look at force_dependencies or in the exp above) */
	if (user_param1->duplex && (user_param1->use_xrc || user_param1->connection_type == DC))
		num_of_qps1 /= 2;

	if (user_param2->duplex && (user_param2->use_xrc || user_param2->connection_type == DC))
		num_of_qps2 /= 2;

	user_param1->tposted[0] = get_cycles();
	user_param2->tposted[0] = get_cycles();

	/* main loop for posting */
	while (1) {
	/* main loop to run over all the qps and post each time n messages */
		for (index =0 ; index < num_of_qps1 ; index++) {

			while ((ctx1->scnt[index] - ctx1->ccnt[index]) < user_param1->tx_depth) {
				if (ctx1->send_rcredit) {
					uint32_t swindow = scnt_for_qp[index] + user_param1->post_list - ctx1->credit_buf[index];
					if (swindow >= user_param1->rx_depth)
						break;
				}

				if (user_param1->post_list == 1 && (ctx1->scnt[index] % user_param1->cq_mod == 0 && user_param1->cq_mod > 1)) {

					#ifdef HAVE_VERBS_EXP
					#ifdef HAVE_ACCL_VERBS
					if (user_param1->verb_type == ACCL_INTF)
						ctx1->exp_wr[index].exp_send_flags &= ~IBV_EXP_QP_BURST_SIGNALED;
					else {
					#endif
						if (user_param1->use_exp == 1)
							ctx1->exp_wr[index].exp_send_flags &= ~IBV_EXP_SEND_SIGNALED;
						else
					#endif
						#ifdef HAVE_IBV_WR_API
						ctx1->qpx[index]->wr_flags &= ~IBV_SEND_SIGNALED;
						#else
						ctx1->wr[index].send_flags &= ~IBV_SEND_SIGNALED;
						#endif
					#ifdef HAVE_ACCL_VERBS
					}
					#endif
				...
				#ifdef HAVE_VERBS_EXP
				if (user_param1->use_exp == 1)
					err = (ctx1->exp_post_send_func_pointer)(ctx1->qp[index],&ctx1->exp_wr[index*user_param1->post_list],&bad_exp_wr);
				else
					err = (ctx1->post_send_func_pointer)(ctx1->qp[index],&ctx1->wr[index*user_param1->post_list],&bad_wr);
				#else
				err = post_send_method(ctx1, index, user_param1);
				#endif
				if (err) {
					fprintf(stderr,"Couldn't post send: %d scnt=%lu \n",index,ctx1->scnt[index]);
					return_value = FAILURE;
					goto cleaning;
				}
				ctx1->scnt[index] += user_param1->post_list;
				scnt_for_qp[index] += user_param1->post_list;
				totscnt += user_param1->post_list;

				/* ask for completion on this wr */
				if (user_param1->post_list == 1 &&
						(ctx1->scnt[index]%user_param1->cq_mod == user_param1->cq_mod - 1 ||
							(user_param1->test_type == ITERATIONS && ctx1->scnt[index] == user_param1->iters - 1))) {
					#ifdef HAVE_VERBS_EXP
					#ifdef HAVE_ACCL_VERBS
					if (user_param1->verb_type == ACCL_INTF)
						ctx1->exp_wr[index].exp_send_flags |= IBV_EXP_QP_BURST_SIGNALED;
		...
		if (totccnt < totscnt) {
			ne = ibv_poll_cq(ctx1->send_cq,CTX_POLL_BATCH,wc);

			if (ne > 0) {

				for (i = 0; i < ne; i++) {
					if (wc[i].status != IBV_WC_SUCCESS) {
						NOTIFY_COMP_ERROR_SEND(wc[i],ctx1->scnt[(int)wc[i].wr_id],ctx1->scnt[(int)wc[i].wr_id]);
						return_value = FAILURE;
						goto cleaning;
					}
					wc_id = (user_param1->verb_type == ACCL_INTF) ?
							0 : (int)wc[i].wr_id;
					user_param1->iters += user_param1->cq_mod;
					totccnt += user_param1->cq_mod;
					ctx1->ccnt[wc_id] += user_param1->cq_mod;
				}
