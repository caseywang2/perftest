...
int parser(struct perftest_parameters *user_param,char *argv[], int argc)
{
	...
	static int dual_device1_flag=0;
	static int dual_device2_flag=0;
	...
	while (1) {
		static const struct option long_options[] = {
			...
			{ .name = "d1",		.has_arg = 1, .flag = &dual_device1_flag, .val = 1 },
			{ .name = "d2",		.has_arg = 1, .flag = &dual_device2_flag, .val = 1 },
		...
		switch (c) {
			...
			case 0: /* required for long options to work. */
				...
				if (dual_device1_flag){
					printf("optdev1         : %s\n", optarg);
					GET_STRING(user_param->ib_devname1,strdupa(optarg)); break;
				//dual_device1_flag = 0;
				}
				if (dual_device2_flag){
					printf("optdev2         : %s\n", optarg);
					GET_STRING(user_param->ib_devname2,strdup(optarg)); break;
					//dual_device2_flag = 0;
				}
